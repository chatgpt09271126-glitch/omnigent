...
describe("parseEvent — thread settings and response flags", () => {
  it("parses the full per-thread UI settings map", () => {
    expect(
      parseEvent("session.ui_settings", {
        conversation_id: "conv_a",
        ui_settings: { response_flagging: true, future_toggle: false },
      }),
    ).toEqual({
  ...
  it("parses a live mid-stream flag update", () => {
    expect(
      parseEvent("response.flagged", {
        conversation_id: "conv_a",
        response_id: "resp_streaming",
        flagged: true,
        flagged_by: "operator@example.com",
        flagged_at: 123,
      }),
    ).toEqual({
