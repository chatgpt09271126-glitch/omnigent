...
describe("chatStore — handleSessionEvent (session.* events)", () => {
  ...
  describe("thread UI settings and response flags", () => {
    it("routes live full-state updates to a background conversation", () => {
      bindConversationForTest("conv_thread_bg", {
        uiSettings: { response_flagging: false },
      });
      bindConversationForTest("conv_thread_fg", {
        uiSettings: { response_flagging: false },
      });
      ...
      handleSessionEvent(
        {
          ...
          conversationId: "conv_thread_bg",
          uiSettings: { response_flagging: true, future_toggle: false },
        },
        "conv_thread_bg",
      );
      handleSessionEvent(
      ...
      expect(conversationRegistry.peek("conv_thread_bg")!.getState()).toMatchObject({
        uiSettings: { response_flagging: true, future_toggle: false },
        flaggedResponses: {
          resp_streaming: { flaggedBy: "operator@example.com", flaggedAt: 123 },
        },
      });
      expect(useChatStore.getState()).toMatchObject({
        conversationId: "conv_thread_fg",
        uiSettings: { response_flagging: false },
        flaggedResponses: {},
      });
    });

    it("removes a live response flag without disturbing other flags", () => {
      bindConversationForTest("conv_flags", {
        flaggedResponses: {
          resp_keep: { flaggedBy: null, flaggedAt: 10 },
          resp_clear: { flaggedBy: "operator@example.com", flaggedAt: 11 },
        },
      });
      ...
      expect(useChatStore.getState().flaggedResponses).toEqual({
        resp_keep: { flaggedBy: null, flaggedAt: 10 },
      });
    });
...
describe("chatStore — session status reconciled when the tab returns visible", () => {
  ...
  it("recovers UI settings and response flags missed during an SSE gap", async () => {
    const id = "conv_vis_flags";
    seedSession(id, []);
    let snapshotCount = 0;
    fetchMock.mockImplementation((input: RequestInfo | URL, init?: RequestInit) => {
      const url = typeof input === "string" ? input : input.toString();
      if (url.split("?")[0] === `/v1/sessions/${id}` && (init?.method ?? "GET") === "GET") {
        const reconnect = snapshotCount > 0;
        snapshotCount += 1;
        return mockResponse({
          id,
          agent_id: "agent_xyz",
          ...
          items: [],
          labels: {},
          ui_settings: { response_flagging: reconnect },
...
describe("chatStore — pumpStreamEvents end reasons", () => {
  ...
  it("returns 'dropped' once the silent-stall watchdog trips (no bytes, no error, no close)", async () => {
    ...
    try {
      ...
      const done = pumpStreamEvents(
        "conv_stall",
        withStallGuard(sink.stream, { stallMs: 1_000 }),
        new AbortController(),
        setState,
        getState,
        immediate,
      );
      // No bytes ever arrive — not even a heartbeat — so the v0.11 guard
      // cancels the body and the pump observes an ordinary dropped stream.
...
describe("chatStore — startStreamPump reconnect loop", () => {
  ...
  it("keeps a heartbeat-alive stream connected across many stall windows", async () => {
    ...
    // Several stall windows of wall clock, with the server's 2-second
    // heartbeat inside each one: the guard must keep re-arming rather than
    // tripping.
    /* oxlint-disable no-await-in-loop */
  ...
  it("recomputes the idle flag on every reconnect, so a backgrounded tab reports idle", async () => {
    ...
    document.dispatchEvent(new Event("visibilitychange"));
    // Model the real server heartbeat while the 30-second presence debounce
    // elapses so this test isolates the idle-triggered recycle from the
    // independent byte-silence guard.
    /* oxlint-disable no-await-in-loop */
    for (let elapsed = 0; elapsed < PRESENCE_IDLE_AFTER_MS; elapsed += 2_000) {
      sinks[0]!.push(sse("session.heartbeat", {}));
      await vi.advanceTimersByTimeAsync(2_000);
    }
    /* oxlint-enable no-await-in-loop */
