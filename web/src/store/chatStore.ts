...
export const useChatStore = create<ChatState>((_rootSet, get) => ({
  ...
  updateUiSetting: async (key, value) => {
    const { conversationId } = get();
    if (!conversationId) return;
    const previous = setterForState(conversationId)?.uiSettings ?? get().uiSettings;
    const patchSet = setterFor(conversationId);
    // Optimistic flip so the popover switch responds instantly; the merge-PATCH
    // response (or the rollback below) is the settled truth. Pin the write to
    // this conversation because the request can finish after a navigation.
    patchSet({ uiSettings: { ...previous, [key]: value } });
    try {
      const session = await updateSession(conversationId, { uiSettings: { [key]: value } });
      patchSet({ uiSettings: session.uiSettings ?? {} });
    } catch (err) {
      patchSet({ uiSettings: previous });
      throw err;
    }
  },
...
async function reconcileSessionStatus(id: string): Promise<void> {
  ...
  if (isConversationDisposed(id)) return;
  setterFor(id)((s) => reconnectStatusPatch(session, s));
}
...
// Two missed 2 s server heartbeats plus slack. Deliberately shorter than
...
export const SSE_STALE_RECYCLE_MS = 6_000;
  ...
  try {
    while (!controller.signal.aborted && !isConversationDisposed(id)) {
      ...
      try {
        ...
        // heartbeats every 2 s, so a longer gap means a half-open socket
...
export function handleSessionEvent(event: StreamEvent, streamConversationId?: string): void {
  ...
  switch (event.type) {
    ...
    case "session_ui_settings":
      ...
      applyToNamedConversation(event.conversationId, { uiSettings: event.uiSettings });
      return;
